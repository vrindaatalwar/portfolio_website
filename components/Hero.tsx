...
const Hero: React.FC = () => {
  ...
  const socialLinks = [
    ...
    { name: 'Email me', url: 'mailto:vrindaatalwar@gmail.com' },
  ];
            ...
            <div className="flex items-center gap-6">
                {socialLinks.map((item) => (
                    <a 
                        key={item.name} 
                        href={item.url}
                        target={item.name.startsWith('Email') ? undefined : "_blank"}
                        rel={item.name.startsWith('Email') ? undefined : "noopener noreferrer"}
